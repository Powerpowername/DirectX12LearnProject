...
void LoadAsset()
{
	...
	}
	{
		...
		psoDesc.DepthStencilState.DepthEnable = FALSE;//不启用深度测试
		psoDesc.DepthStencilState.StencilEnable = FALSE;//不启用模板测试
	...
	ThrowIfFailed(commandList->Close());
	{
		...
		ThrowIfFailed(vertexBuffer->Map(0, &readRange, reinterpret_cast<void**>(&pVertexDataBegin)));//映射缓冲区到CPU可读内存
		memcpy(pVertexDataBegin, triangleVertices, sizeof(triangleVertices));//将顶点数据复制到缓冲区

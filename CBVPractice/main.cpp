...
UINT* pCbvDataBgegin = nullptr;//CBV数据起始地址
...
ComPtr<ID3D12Resource> constantBuffer;//常量缓冲区
SceneConstantBuffer constantBufferData;//常量缓冲区数据
UINT8* pCbvDataBegin;//常量缓冲区数据起始地址
//同步对象
...
//创建描述符堆
{
	...
	cbvHeapDesc.NumDescriptors = 1;//常量缓冲区描述符数量
	cbvHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;//常量缓冲区、着色器资源视图和无序访问视图
	cbvHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE; //这个描述符堆是可见的，GPU可以直接访问
...
{ { 0.0f, 0.75f, 0.7f }, { 0.0f, 0.0f, 1.0f, 1.0f } }
		...
		*/
		...
		const UINT constantBufferSize = CalcConstantBufferSize<SceneConstantBuffer>();
		CD3DX12_RESOURCE_DESC cosntantResourceDesc = CD3DX12_RESOURCE_DESC::Buffer(constantBufferSize);
		ThrowIfFailed(device->CreateCommittedResource(
			&heapProperties,
			D3D12_HEAP_FLAG_NONE,
			&cosntantResourceDesc,
			D3D12_RESOURCE_STATE_GENERIC_READ,
			nullptr,
		...
		device->CreateConstantBufferView(&cbvDesc, cbvHeap->GetCPUDescriptorHandleForHeapStart());//创建常量缓冲区视图

		ThrowIfFailed(constantBuffer->Map(0, &readRange, reinterpret_cast<void**>(&pCbvDataBegin)));
		memcpy(pCbvDataBegin, &constantBufferData, sizeof(constantBufferData)); //将常量缓冲区数据复制到缓冲区

...
#include "d3dUtil.h"
#include "d3dx12.h"
#include <DirectXPackedVector.h>//DirectXMath库，数学库
...
void LoadPipeline()
{
	...
	ComPtr<IDXGIFactory4> mDxgiFactory;// DXGI工厂
	...
	CD3DX12_CPU_DESCRIPTOR_HANDLE rtvHandle(rtvHeap->GetCPUDescriptorHandleForHeapStart());// 获取RTV描述符堆的CPU描述符句柄
...
void OnRender()
{
	...
	ThrowIfFailed(swapChain->Present(1, 0));//第一个参数1表示垂直同步，0表示不垂直同步，垂直同步可以减少画面撕裂现象

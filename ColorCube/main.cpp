#include <Windows.h>
#include <d3d12.h> //高层 API，直接操作 GPU 进行渲染
#include <dxgi1_4.h>//底层 API，为上层 API（如 D3D12、D3D11）提供支持
#include <iostream>
#include "d3dx12.h"
...
IDXGIAdapter1* GetSupportedAdapter(ComPtr<IDXGIFactory4>& dxgiFactory, const D3D_FEATURE_LEVEL featureLevel)
{
	...
	for (INT adapterIndex = 0;;++adapterIndex)
	{
		...
		//只是查找，并没有真的去建立，所以最后一个参数是nullptr
		const HRESULT hres = D3D12CreateDevice(currentAdapter, featureLevel, _uuidof(ID3D12Device), nullptr);
		if (SUCCEEDED(hres))//其实找的是第一个符号要求的显示适配器
		{
			adapter = currentAdapter;
			break;
		}
		currentAdapter->Release();//不是显示适配器设备就释放com资源
	}
	return adapter;
}
...
void LoadPipeline()
{
	...
	if (adapter != nullptr)
	{
		D3D12CreateDevice(adapter, featureLevels[selectfeature], IID_PPV_ARGS(device.GetAddressOf()));
	}
...
{
	{
		...
		//此处只是通过根签名申明常量缓冲区的数据要放在指定的寄存器上，但是真正的绑定需要SetGraphicsRootDescriptorTable把描述符堆的首地址交割描述符表
	...
	}
	{
		...
		UINT compileFlags = 0;
#endif
		ThrowIfFailed(D3DCompileFromFile(std::wstring(L"../Assets/set_register_shaders.hlsl").c_str(), nullptr, nullptr, "VSMain", "vs_5_0", compileFlags, 0, &vertexShader, nullptr));
		ThrowIfFailed(D3DCompileFromFile(std::wstring(L"../Assets/set_register_shaders.hlsl").c_str(), nullptr, nullptr, "PSMain", "ps_5_0", compileFlags, 0, &pixelShader, nullptr));
...
void PopulateCommandList()
{
	...
	//着色其可见的描述符堆需要SetDescriptorHeaps
	ID3D12DescriptorHeap* ppHeaps[] = { cbvHeap.Get() };
	commandList->SetDescriptorHeaps(_countof(ppHeaps), ppHeaps);//告诉 GPU 哪些描述符堆是着色器可见的
	commandList->SetGraphicsRootDescriptorTable(0, cbvHeap->GetGPUDescriptorHandleForHeapStart());//告诉 GPU 具体使用这些堆中的哪些描述
	...
	commandList->IASetVertexBuffers(0, 1, &vertexBufferView);//指定顶点数据来源的顶点缓冲
